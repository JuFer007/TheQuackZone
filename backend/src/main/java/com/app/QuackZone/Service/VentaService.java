...
public class VentaService {
    ...
    public VentaService(VentaRepository ventaRepository, CarritoRepository carritoRepository, UsuarioRepository usuarioRepository, EmailService emailService) {
    ...
    public Venta generarVenta(Long usuarioId) {
        ...
        List<Juego> juegos = carrito.getJuegos();
        Map<Long, Double> preciosMap = obtenerPreciosConDescuento(juegos);

        Venta venta = new Venta();
        venta.setUsuario(usuario);
        venta.setVideojuegos(new ArrayList<>(juegos));
        venta.setFecha(LocalDate.now());

        double total = juegos.stream()
            .mapToDouble(j -> preciosMap.getOrDefault(j.getId(), j.getPrecio() != null ? j.getPrecio() : 0.0))
            .sum();
        venta.setTotal(total);

        Venta ventaGuardada = ventaRepository.save(venta);

        emailService.enviarConfirmacionCompra(usuario, ventaGuardada, juegos, preciosMap);
    ...
    public Venta generarVentaPorCarrito(Long carritoId) {
        ...
        emailService.enviarConfirmacionCompra(carrito.getUsuario(), ventaGuardada, juegos, preciosMap);
    ...
    private Map<Long, Double> obtenerPreciosConDescuento(List<Juego> juegos) {
        Map<Long, Double> preciosMap = new HashMap<>();
        try {
                ...
                for (Juego j : juegos) {
                    ...
                    if (dealObj instanceof Map) {
                        ...
                        if (priceObj != null) {
                            ...
                            preciosMap.put(j.getId(), salePrice);
                        }
                    }
                }
            }
        } catch (Exception e) {
            System.err.println("[VentaService] Error al obtener descuentos: " + e.getMessage());
        }
        // Fallback: precio original si no hay descuento
        for (Juego j : juegos) {
            preciosMap.putIfAbsent(j.getId(), j.getPrecio() != null ? j.getPrecio() : 0.0);
        }
        return preciosMap;
    }
